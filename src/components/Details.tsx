...
function PlayingCard({ rank, suit, red, title, children, dealDelayMs = 0, floatDelayMs = 0 }: {
  rank: string;
  suit: string;
  red?: boolean;
  title: string;
  ...
  return (
    <Reveal delayMs={dealDelayMs}>
      <div
        className="card-float [perspective:900px]"
  ...
  return (
    <section className="w-full" aria-labelledby="details-heading">
      ...
      <div className="grid grid-cols-2 gap-4 sm:gap-6 md:grid-cols-4">
        <PlayingCard rank="A" suit="♥" red title="The date" dealDelayMs={0} floatDelayMs={0}>{CONFIG.date}</PlayingCard>
        <PlayingCard rank="K" suit="♠" title="The time" dealDelayMs={120} floatDelayMs={300}>{CONFIG.time}</PlayingCard>
        <PlayingCard rank="Q" suit="♦" red title="The place" dealDelayMs={240} floatDelayMs={600}>
          {CONFIG.locationTeaser}
          <br />
          <span className="text-zinc-500">Revealed when you RSVP</span>
        </PlayingCard>
        <PlayingCard rank="J" suit="♣" title="The dress" dealDelayMs={360} floatDelayMs={900}>{CONFIG.dressCode}</PlayingCard>
      </div>
    </section>
  );
}
